...
- **`name`** — the extension's existing `name` is a free-form project
  ...
  for dependency resolution to work (a consumer's `pope_dependencies` map is
  ...
  `pope_package_name` key instead, leaving the extension's `name` untouched.
...
original name of what's now `pope_dependencies`) collided with a
...
map. Fixed by renaming to `pope_dependencies`, and — since a schema this
...
principle, `pope_package_name` included, even though `package_name` itself
...
- Whether scope stays OO-ABL-only ([ADR-0002](../decisions/0002-oo-abl-only-v1.md))
  — if that changes, `pope_package_name`'s namespace-directory semantics here
...
- `pope_package_name` uniqueness is now enforced within a single registry
  directory: `LocalDirectoryRegistry.findAny` fails loudly if two folders
  both declare the same `pope_package_name`, rather than silently picking
