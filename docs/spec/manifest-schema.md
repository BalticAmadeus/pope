...
| `popeToolVersion` | pope (new) | The pope version that last wrote this manifest — auto-stamped by `ManifestWriter` on every write, never hand-authored. Null/absent when a manifest predates this field, or was written under `includeBuild` (no packaged jar to read the running version from). `popeInstall` warns (doesn't fail) when a resolved dependency's recorded major version differs from the currently-running pope's major — see `pope.PopeVersion.majorMismatch`. |
