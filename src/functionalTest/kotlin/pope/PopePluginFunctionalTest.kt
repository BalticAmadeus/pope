...
class PopePluginFunctionalTest {
    ...
    fun `popeInstall and popePropath run for real against the fixture packages`() {
        ...
        val fixtureManifest =
            JSONObject()
                ...
                .put("version", "1.0.0")
                .put("popePackageName", "example.consumer")
    ...
    fun `popeInstall -PpopeAdd adds and resolves a dependency in one step, without a prior manifest edit`() {
        ...
        val dependencies = JSONObject(File(projectDir, "openedge-project.json").readText()).getJSONObject("popeDependencies")
        assertTrue(
    ...
    private fun buildMergedRegistriesProject(): File {
        ...
        projectDir.resolve("openedge-project.json").writeText(
            JSONObject()
                ...
                .put("version", "1.0.0")
                .put("popePackageName", "example.merge")
                .put("popeDependencies", JSONObject().put("z.something", "^1.0.0"))
    ...
    fun `pope_packages nests a catalog-routed package under its registry's own name, and a direct-source one under the shared dependencies root`() {
        ...
        File(calculatorRepo, "openedge-project.json").writeText(
            JSONObject()
                ...
                .put("version", "1.0.0")
                .put("popePackageName", "calculator")
                .put(
                    "popeDependencies",
                    JSONObject().put(
                        "greeter",
        ...
        projectDir.resolve("openedge-project.json").writeText(
            JSONObject()
                ...
                .put("version", "1.0.0")
                .put("popePackageName", "example.consumer")
                .put("popeDependencies", JSONObject().put("ba.calculator", "^1.0.0"))
                .put("buildPath", JSONArray().put(JSONObject().put("type", "source").put("path", "src")))
                .toString(2),
        )
    ...
    fun `an explicit registryName-localName dependency resolves via that registry, nested under its shared registry root`() {
        ...
        projectDir.resolve("openedge-project.json").writeText(
            JSONObject()
                ...
                .put("version", "1.0.0")
                .put("popePackageName", "example.consumer")
                .put("popeDependencies", JSONObject().put("registry-ba/calculator", "^1.0.0"))
                .put("buildPath", JSONArray().put(JSONObject().put("type", "source").put("path", "src")))
                .toString(2),
        )
    ...
    fun `popePrune removes one dropped package's files while its sibling survives, then the registry root once both are gone`() {
        ...
        // Only patches "popeDependencies" on the manifest already on disk - real usage never touches
        ...
        fun setDependencies(dependencies: JSONObject) {
            ...
            val json = JSONObject(manifestFile.readText()).put("popeDependencies", dependencies)
    ...
    fun `a dependency's own test entries are never copied into pope_packages or added to a consumer's buildPath`() {
        ...
        packageDir.resolve("openedge-project.json").writeText(
            JSONObject()
                ...
                .put("version", "1.0.0")
                .put("popePackageName", "example.hastests")
                .put("popeDependencies", JSONObject())
                .put(
                    "buildPath",
                    JSONArray()
        ...
        val manifest =
            JSONObject()
                ...
                .put("version", "1.0.0")
                .put("popePackageName", "example.consumer")
                .put("popeDependencies", JSONObject().put("example.hastests", "^1.0.0"))
                .put("buildPath", JSONArray().put(JSONObject().put("type", "source").put("path", "src")))
        val projectDir = buildProject(registryDir, manifest)
    ...
    private fun writeManifest(projectDir: File, dependencyNames: List<String>) {
        ...
        // Only touches "popeDependencies" - a fresh manifest the first time
        ...
        for (name in listOf("alpha", "beta")) {
            ...
            packageDir.resolve("openedge-project.json").writeText(
                JSONObject()
                    ...
                    .put("version", "1.0.0")
                    .put("popePackageName", "example.$name")
                    .put("popeDependencies", JSONObject())
                    .put("buildPath", JSONArray().put(JSONObject().put("type", "source").put("path", "src")))
                    .toString(2),
            )
    ...
    fun `popeUninstall removes a dependency and its pope_packages, buildPath, and lock entries, leaving others alone`() {
        ...
        assertTrue(
            !JSONObject(File(projectDir, "openedge-project.json").readText()).getJSONObject("popeDependencies").has("example.beta"),
        ...
        assertTrue(
            JSONObject(File(projectDir, "openedge-project.json").readText()).getJSONObject("popeDependencies").has("example.alpha"),
